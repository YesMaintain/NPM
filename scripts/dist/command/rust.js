...
`)}if(a.size>1){try{await w(`${r}${o}`,{recursive:!0})}catch{console.log(`Could not create: ${r}${o}`)}try{await $(`${r}${o}${t}`,`${[...a].join("")}`)}catch{console.log(`Could not create workflow for: ${r}/workflows/rust.yml`)}}else try{await g(`${r}${o}${t}`,m.F_OK);try{await p(`${r}${o}${t}`)}catch{console.log(`Could not remove ${o}${t} for: ${r}`)}}catch{}}};var B=async()=>{await v(b)};export{B as default};
