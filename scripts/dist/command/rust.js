...
`)}if(e.size>1){try{await a.promises.mkdir(`${r}${o}`,{recursive:!0})}catch{console.log(`Could not create: ${r}${o}`)}try{await a.promises.writeFile(`${r}${o}${t}`,`${[...e].join("")}`)}catch{console.log(`Could not create workflow for: ${r}/workflows/rust.yml`)}}else try{await a.promises.access(`${r}${o}${t}`,a.constants.F_OK);try{await a.promises.rm(`${r}${o}${t}`)}catch{console.log(`Could not remove ${o}${t} for: ${r}`)}}catch{}}};var F=async()=>{await d(y)};export{F as default};
