...
`)}if(a.size>0){try{await e.promises.mkdir(`${t}${o}`,{recursive:!0})}catch{console.log(`Could not create: ${t}${o}`)}try{await e.promises.writeFile(`${t}${o}${r}`,`${[...a].join("")}`)}catch{console.log(`Could not create workflow for: ${t}/dependabot.yml`)}}else try{await e.promises.access(`${t}${o}${r}`,e.constants.F_OK);try{await e.promises.rm(`${t}${o}${r}`)}catch{console.log(`Could not remove ${o}${r} for: ${t}`)}}catch{}}};var j=async()=>{await y(g)};export{j as default};
