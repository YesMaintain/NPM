import * as a from "fs";
import { basename as p, dirname as w } from "path";
import g from "../lib/git-directories.js";
import $ from "../lib/package-types.js";
import u from "../lib/packages.js";
import y from "../options/rust.js";
const d=async f=>{for(const{path:o,name:t,workflow:m}of f)for(const[i,l]of await g(await u())){const r=`${i}/.github`,e=await m();if(o==="/workflows/"&&t==="rust.yml")for(const s of l){const c=w(s).replace(i,""),n=(await $()).get(s.split("/").pop());typeof n<"u"&&n==="cargo"&&e.add(`
